	...
	<div class="mt-10 sm:mx-auto sm:w-full sm:max-w-[480px]">
		<div class="bg-white px-6 py-12 shadow sm:rounded-lg sm:px-12">
			<form class="space-y-6">
				...
				<div>
					{#if !isSubmitting}
						<Button class="w-full" on:click={authenticate} type="submit">Sign in</Button>
					{:else}
			...
			{#if !formSuccess && formMessage}
				<div class="flex justify-center">
					<p class="mt-4 text-sm font-medium text-red-600">{formMessage}</p>
				</div>
			{/if}
		</div>
